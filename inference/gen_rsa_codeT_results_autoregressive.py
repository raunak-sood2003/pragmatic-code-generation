...
def generate_rsa_codeT_results(num_programs, num_tests, num_input_tests, num_ransac_samples, num_out_programs, \
    ...
    for i, task_id in tqdm(enumerate(task_ids)):
        ...
        # Generating RSA test cases
        test_idx_map = {test : idx for idx, test in enumerate(tests)}
        rsa_testcases = []
        const_matrix_update = np.copy(const_matrix)
        programs_update = programs.copy()
        tests_update = tests.copy()
        for _ in range(num_input_tests):
            # Inference time RSA step
            if const_matrix_update.size == 0:
                break

            L0 = RSA.normalize_rows(const_matrix_update)
            S1 = RSA.normalize_cols(L0)

            prag_testcases = np.zeros([len(tests_update)])
            for j in range(len(programs_update)):
                logprobs = program_prob_map[programs_update[j]]
            ...
            rsa_testcase_idx = np.argmax(prag_testcases)
            rsa_testcase = tests_update[rsa_testcase_idx]
            rsa_testcases.append(rsa_testcase)
            # Auto-regressively update the const matrix
            include_programs = const_matrix_update[rsa_testcase_idx, :] == 1
            const_matrix_update = const_matrix_update[:, include_programs]
            programs_update = np.array(programs_update)[include_programs].tolist()
            
            include_tests = np.ones(const_matrix_update.shape[0], dtype = bool)
            include_tests[rsa_testcase_idx] = 0
            const_matrix_update = const_matrix_update[include_tests, :]
            tests_update = np.array(tests_update)[include_tests].tolist()

        if len(rsa_testcases) > 0:
            rsa_testcase_idxs = [test_idx_map[test] for test in rsa_testcases]
            tests = np.array(tests)[np.array(rsa_testcase_idxs)].tolist()
            const_matrix = const_matrix[np.array(rsa_testcase_idxs), :]
            ...
            # If no CodeT, run CodeT on the original tests
            codet = CodeT(programs, tests, num_ransac_samples, num_out_programs, const_matrix)
